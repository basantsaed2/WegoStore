...
const [selectedOrders, setSelectedOrders] = useState([]);
        ...
        if (selectedOption && selectedPaymentId) {
          ...
          try {
            ...
            if (selectedOption === "Approve") {
              response = await axios.post(` https://bcknd.wegostores.com/public/admin/v1/payment/approve/${selectedPaymentId}`, {}, {
                headers: {
                  Authorization: `Bearer ${auth.user.token}`,
                },
              });
            } else if (selectedOption === "Reject") {
              response = await axios.post(` https://bcknd.wegostores.com/public/admin/v1/payment/rejected/${selectedPaymentId}`,
                {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
               const response = await axios.get(' https://bcknd.wegostores.com/public/admin/v1/payment/show/pending', {
                      headers: {
                             Authorization: `Bearer ${auth.user.token}`,
                      },
               });
