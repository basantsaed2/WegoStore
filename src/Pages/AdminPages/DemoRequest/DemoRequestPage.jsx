...
const DemoRequestPage = () => {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const response = await axios.get(' https://bcknd.wegostores.com/public/admin/v1/demoRequest/show', {
                headers: {
    ...
    const handleSubmit = async () => {
        try {
            ...
            const response = await axios.post(
                ` https://bcknd.wegostores.com/public/admin/v1/demoRequest/approved/${selectedRequest.id}`,
                formData,
                {
                    headers: {
                        Authorization: `Bearer ${auth.user.token}`,
